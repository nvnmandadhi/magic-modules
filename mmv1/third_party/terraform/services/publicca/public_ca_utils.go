...
func createNewAccountUsingEab(email string, isStagingEnv bool, privateKeyPem string, keyId string, hmacEncoded string) (string, error) {
	...
	eabOptions := registration.RegisterEABOptions{
		...
		Kid:                  keyId,
		HmacEncoded:          convertHmacToBase64Url(hmacEncoded),
	}
...
func getPrivateKey(privateKeyPem string) crypto.PrivateKey {
	if privateKeyPem != "" {
		...
		log.Printf("[DEBUG] creating a new private key of type ECDSA")
		_key, _ := certcrypto.GeneratePrivateKey(certcrypto.RSA2048)
		return _key
	}
}

func convertHmacToBase64Url(hmacB64 string) string {
